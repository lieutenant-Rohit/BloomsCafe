...
A full-stack cafe e-commerce platform with customer-facing storefront and admin management panel.
...
Browser → Nginx (:8080)
  ...
  └── /api/*   → round-robin to App Instance (:8081/:8082/:8083)
...
|---------|---------------|
| **Horizontal scaling** | Nginx round-robin across 3+ Spring Boot instances. Run with `make run3`. |
| **Caching** | Redis caches products and categories — TTL of 10 minutes. Cache invalidated on writes. |
| **Read/write split** | `AbstractRoutingDataSource` routes read-only transactions to replica, writes to primary. `LazyConnectionDataSourceProxy` defers connection acquisition. |
| **N+1 query prevention** | All repositories use `JOIN FETCH` and batch fetching. Documented in `docs/(N+1)QUERY.md`. |
...
```
Runs on `http://localhost:8080`. Configure DB credentials in `src/main/resources/application.properties`.
